...
include('../database/db.php');
   ...
   $select_item =$db->prepare("SELECT * FROM item WHERE code_item='$code'");
   $select_item->execute();
   ...
   WHERE stock_id='$stock_id' AND bn_stock ='$bn_name'");
 ...
 if(!empty($errorMsg)){
  $list_item = array('errorMsg'=>$errorMsg);
 }else{
  $list_item = array('stock_id'=>$stock_id,'item_name'=>$item_name,'code_item'=>$code_item,'sum_item'=>$sum,'bn_name'=>$bn_name,'exd_date'=>$exd_date,'bn_id'=>$bn_id,'price_stock'=>$price_stock);
 }
